  ...
  return (
    <Switch.Group as="div" className="flex items-center">
      <Switch.Label as="span" className="mr-3 text-sm">
        <span className={`text-gray-900 ${!isSummary ? 'font-bold' : 'font-medium'}`}>Original Note</span>{' '}
      </Switch.Label>
      <Switch
        checked={isSummary}
      ...
      <Switch.Label as="span" className="ml-3 text-sm">
        <span className={`text-gray-900 ${isSummary ? 'font-bold' : 'font-medium'}`}>AI Summary</span>{' '}
      </Switch.Label>
    </Switch.Group>
  )
}
